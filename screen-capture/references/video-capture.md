...
- **Headless-Chromium codec limitation:** the Playwright-bundled Chromium build has no H.264/AAC decoder (`canPlayType('video/mp4; codecs="avc1..."')` returns empty). `<video>` elements sourcing standard H.264 mp4s will show their poster but never play, and may briefly surface a "Runtime NotSupportedError / Failed to load because no supported source was found" dev overlay. This is an environment artifact, not a site bug — note it in the delivery caption rather than debugging the site.

## Making the mouse visible in the recording

Headless Chromium never paints a cursor, so any click/hover demo is invisible without one. Inject a synthetic cursor element via `context.addInitScript` (so it re-attaches on every navigation, including a PDP landing at the end of a click-through) and drive real, gradual `page.mouse.move` calls rather than jumping straight to click targets:
