...
- **`scripts/`** — the pipeline scripts, one per stage (see the table in `SKILL.md`), plus template utilities: `load_template.py` (source-agnostic vertical-template loader), `scan_image_urls.py` (pre-load image-URL liveness scan), `capture_rendered_images.py` (Playwright fallback for JS/lazy-loaded images), and `replace_product_images.py` (safe delete-and-add image refresh). Constants that must change per site/store are marked `# EDIT PER SITE`.
