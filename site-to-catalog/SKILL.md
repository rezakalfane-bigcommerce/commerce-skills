...
- `scripts/load_template.py` — source-agnostic loader (brands → tree categories →
  ...
  Idempotent and resumable. Proven on 5 verticals.
...
- **trees/categories API shape gotchas:** batch `POST` returns items keyed `category_id` while the `GET` tree view uses `id`; category `url` must be an object `{"path": ...}` (plain string → 422); reruns hit "duplicate name in same parent" unless you reconcile by (name, parent) first. And the tree `GET` nests `children` — don't verify parents by matching `parent_id` across the top-level array. This bit `load_template.py` itself (fixed): its by-name-parent reconciliation only read the top-level array, so already-created leaf categories looked missing and got retried (409/422) on every rerun — always flatten `children` recursively before matching.
- **Product create can return `207` (multi-status) on a genuine success** (e.g. an image sub-resource warning) — `load_template.py` was treating any non-200/201 as a failure, which logged every successful create as `FAILED 207` *and* silently excluded it from the end-of-run channel assignment. Check for `response.data.id`, not just the status code, to decide success.
- **Gate a CSV header write on the file being new, not on an in-memory "nothing loaded yet" flag.** `load_template.py`'s product-map writer opened in append mode and wrote a header whenever its `done` dict was empty — which, combined with the `207` bug above, meant a rerun with zero *recognized* successes wrote a second header row into the resumable state file. Check `file.exists()`/size instead.
